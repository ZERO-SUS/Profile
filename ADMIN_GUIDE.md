...
service cloud.firestore {
  match /databases/{database}/documents {
    match /{document=**} {
      allow read: if true;                  // anyone can read posts & projects
      allow write: if request.auth != null; // only signed-in (you) can change them
    }
  }
}
```

> Tip: copy these from the plain-text file **`firestore.rules`** in the project to
> avoid smart-quote / backtick paste errors. Make sure you're in **Firestore Database
> → Rules**, not *Realtime Database* (that uses a different JSON format).
